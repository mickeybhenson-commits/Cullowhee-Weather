...
def main():
    ...
    print("      ^ gribble_gap.json says 10-min. MEASURED from the record: 5.0 min to")
    print("        2016-04, 10.0 min after. The JSON is wrong for the first third; that")
    print("        error is what produced the runoff_in bug corrected in Part 1.")
    ...
    if all_k:
        ...
        print()
        print("  AND the observed side is a lower bound — unevenly. The record samples every")
        print("  5.0 min until 2016-04 and every 10.0 min after, against a 9.7-min Tc. So")
        print("  post-2016-05 peaks are sampled at roughly the catchment's own response time")
        print("  and can rise and fall between samples; pre-2016-05 peaks are resolved about")
        print("  twice as well. k is therefore biased LOW, and biased UNEVENLY across the")
        print("  record, by an amount this data cannot quantify. Treat the span as an")
        print("  argument, not a measurement — and do not compare early k against late k")
        print("  without accounting for it.")
    ...
    print()
    print("  A third reason was raised on 2026-08-16 and then CLOSED the same day, which is")
    print("  worth stating rather than quietly dropping. runoff_in did not have one")
    print("  consistent relationship to the record it came from; the cause turned out to be")
    print("  a derivation that assumed a fixed 10-min step over a record that samples at")
    print("  5 min until 2016-04. Exact factor, correctable, and corrected in Part 1. It is")
    print("  no longer a reason Mode C cannot be fitted.")
    print()
    print("  What it leaves behind is smaller but real: the inventory's derivation was not")
    print("  checked against its own source until now, and peak_cfs was only confirmed")
    print("  because a maximum does not depend on the time step. Any FUTURE column derived")
    print("  from this record should be checked the same way before it is relied on.")
    print("  See claude/noah_gribble_observed_verified_2026-08-16.md.")
    print()
