"""
Test Model — Streamlit page for the Cullowhee flood engine (live weather only).
Imports test_model.py from the repo root (no logic duplicated here).

Single Live weather tab: real forecast + modeled antecedent driving the eight
sub-basins, a looping NEXRAD radar, and an all-directions approach-rainfall table.
Only PRF is exposed (sidebar). Runs on live data — no scenario knobs.
"""
...
st.title("Cullowhee Creek flood engine — live")
st.caption("Real forecast + modeled antecedent driving the eight nested sub-basins. "
           "No sensors yet — shadow mode. Placeholders live in test_model.py.")
...
with st.sidebar:
    ...
    st.caption("PRF tunes the unit-hydrograph peak. This page runs on live "
               "weather — no scenario knobs.")

tab4, = st.tabs(["Live weather"])
