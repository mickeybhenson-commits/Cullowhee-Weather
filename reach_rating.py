...
the FIMAN Monitor standard adopted watershed-wide: 0.3 x the 2-yr regression flow (flood_rating.WATCH_MONITOR_FRACTION;
calibrated at the gage, where FIMAN's Monitor 4.0 ft = ~240 cfs = 0.29 x Speedwell's 2-yr). Countdown and status
pill therefore always agree. The section's own lower bank and the registry's surveyed bankfull
...
BED_BELOW_WS = 1.0
WATCH_FRACTION = 0.3   # = flood_rating.WATCH_MONITOR_FRACTION
...
def build(step=0.1, dmax=14.0):
    ...
    out = dict(source=d["source"], built="2026-09-07", method="conveyance-weighted Manning through a 3DEP LiDAR section at the pour point; "
               "depth above the inferred thalweg (parabolic bed 1 ft below the LiDAR water surface); WATCH = stage at 0.3 x the 2-yr flow (FIMAN Monitor standard) through the same rating",
