...
# NOTE: google.cloud.firestore is imported lazily inside get_db(), not here.
# sources.py states the contract — "the Firestore client is imported lazily so
# this module loads with or without google-cloud-firestore installed" — and an
# eager import here broke it: the whole app died at import time whenever the
# package was missing or its transitive crypto bindings failed to load, even
# though Firestore only feeds one optional stage series (fetch_stage_series,
# which already degrades to None).
...
with st.container(border=True):
    ...
    try:
        ...
        _deck = pdk.Deck(layers=_layers, initial_view_state=_view,
                         ...
                         tooltip={"text": "{name}\n{tip}"})
        st.pydeck_chart(_deck)   # width defaults to "stretch" (was use_container_width=True)
...
with st.expander("Reach detail \u2014 depth, discharge, drainage area"):
    ...
    st.dataframe(
        ...
        hide_index=True)   # width defaults to "stretch" (was use_container_width=True)

with st.expander("Schematic view (no basemap)"):
    # st.iframe replaces the deprecated components.v1.html (removal date passed).
    # The SVG is generated by flood_profile.corridor_svg(), not user input.
    st.iframe(
